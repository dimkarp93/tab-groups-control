...
- **Язык интерфейса.** Расширение локализовано штатным механизмом `_locales`: английский — язык по умолчанию (`default_locale: "en"`), русский подхватывается автоматически, если язык интерфейса Firefox русский. Любая другая локаль получает английские тексты. Отдельного переключателя в расширении нет: язык меняется языковым пакетом Firefox (`about:preferences` → Язык) — переводятся и попап с диалогами, и нотификации, и описания команд в **Управлении горячими клавишами расширений**, а даты снимков форматируются по той же локали.
